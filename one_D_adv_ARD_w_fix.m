function [T_np1_k_j,phi_np1_k_j,S_np1_k_j,w_np1_k_j,S_ice_add]=one_D_adv_ARD_w_fix(...
    k_i,k_br,k_s,dt,dz,H,c_i,c_br,L,Tol,phi_initial,T_initial,...
    S_initial,S_bottom,rho_i,rho_br,Tm,m,w_initial,n,Ttop,Tbottom,k_ibr,Salt_In_Ice)
...
while Err>Tol;
    ...
    [T_source,S_source]=one_D_advective_flux(phi_np1_km2_j,S_np1_km1_j...
    ,T_np1_km1_j,H,dz,dt,S_bottom);
    
    delta_T=1.853*(S_np1_km1_j/28);        %% Freezing point depression due to salt
    ...
    En=c_i*T_np1_km1_j(i)+L*phi_np1_km2_j(i);
        if En<Hs(i)
            ...
            phi_np1_km1_j(i)=0;
        elseif En>Hs(i)+L;
            phi_np1_km1_j(i)=phi_np1_km2_j(i);
        else
    ...
    for i=1:matrix_dimension;
        ...
        else a2(i)=1+2*D_s_mod(i)-D_w_s(i);
...
%    end
    ...
    if counter==1;
        ...
        Err_T=max(T_evolve(:,counter)-T_evolve(:,counter-1));
        Err_phi=max(phi_evolve(:,counter)-phi_evolve(:,counter-1));
        Err_S=max(S_evolve(:,counter)-S_evolve(:,counter-1));
...
%S_np1_k_j(expulsion_element+1)=S_np1_k_j(expulsion_element+1)+phi_np1_km1_j(expulsion_element+1)*salts;
for i=1:matrix_dimension;
if phi_np1_km1_j(i)-phi_initial(i)<=0;
    S_ice_add(i)=-(rho_i/rho_br)*(S_initial(i)*((1/k_ibr)))...
        .*(phi_np1_km1_j(i)-phi_initial(i));
else
    S_ice_add(i)=-(rho_i/rho_br)*(Salt_In_Ice(i))...
        .*(phi_np1_km1_j(i)-phi_initial(i));
end
end
